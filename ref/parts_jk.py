...
def polish(im, ncol=7):
    """flat tones + a dark outline around the silhouette, like a fighting-game sprite"""
    px = im.load(); w, h = im.size
    pts = [px[x, y][:3] for y in range(h) for x in range(w) if px[x, y][3]]
    if len(pts) < 8: return im
    if ncol:
        ...
        pal = [c for c in pal if lum(c) > 38] + [OUT]
        cache = {}
        def snap(c):
            if c not in cache: cache[c] = OUT if lum(c) <= 40 else min(pal, key=lambda q: sum((q[i] - c[i]) ** 2 for i in range(3)))
            return cache[c]
        for y in range(h):
            for x in range(w):
                p = px[x, y]
                if p[3]: px[x, y] = snap(p[:3]) + (255,)
    else:
        for y in range(h):
            for x in range(w):
                p = px[x, y]
                if p[3] and lum(p) <= 40: px[x, y] = OUT + (255,)
    # despeckle: a pixel with no opaque 4-neighbour goes
    for y in range(h):
        for x in range(w):
            if px[x, y][3] and not any(0 <= x + dx < w and 0 <= y + dy < h and px[x + dx, y + dy][3] for dx, dy in ((1, 0), (-1, 0), (0, 1), (0, -1))):
                px[x, y] = (0, 0, 0, 0)
    # outline only against the outside (transparent pixels reachable from the border), not around inner holes
