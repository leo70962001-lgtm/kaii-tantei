...
(function (root) {
  ...
  function paintNear() {
    ...
    TREES = layer(W, H);
    const sakura = (cx, cy, rx, ry, seed, T) => {
      T = T || L;
      if (T === L) for (let y = cy + ry * 0.5; y < 190; y++) for (let i = -3; i <= 3; i++) { T.set(cx + i, y, i < -1 ? hx('#3a2a38') : i > 1 ? hx('#1a1020') : hx('#2a1c30')); TREES.set(cx + i, y, 1); }   // the trunk (the front-layer canopies hang in from above)
      const GAP = hx('#2a1c30');
      ellipse(T, cx, cy, rx * 0.86, ry * 0.86, hx('#3a2240')); if (T === L) ellipse(TREES, cx, cy, rx * 0.86, ry * 0.86, 1);
      for (const [bx, by] of [[-rx * 0.6, -ry * 0.3], [rx * 0.55, -ry * 0.45], [-rx * 0.25, -ry * 0.8], [rx * 0.2, -ry * 0.9], [rx * 0.75, ry * 0.05], [-rx * 0.8, ry * 0.1]]) { line(T, cx, cy + ry * 0.4, cx + bx, cy + by, GAP); line(T, cx + 1, cy + ry * 0.4, cx + bx + 1, cy + by, hx('#1a1020')); if (T === L) { line(TREES, cx, cy + ry * 0.4, cx + bx, cy + by, 1); line(TREES, cx + 1, cy + ry * 0.4, cx + bx + 1, cy + by, 1); } }
      ...
      for (const [px, py, pr] of clumps) {
        for (let y = py - pr - 1; y <= py + pr + 1; y++) for (let x = px - pr - 1; x <= px + pr + 1; x++) {
          ...
          if (T === L) TREES.set(x, y, 1);
          if (d > 0.9) { T.set(x, y, l < 0.2 || dith(x, y, 0.5) ? RIM : SH); continue; }
          const c = l > 0.5 ? HI : l > 0.05 ? LIT : l > -0.45 ? MID : SH;
          T.set(x, y, HS(x, y, seed + 6) < 0.04 ? hx('#fff0f4') : c);
        }
      }
    };
  ...
  function paintFront() {
    ...
    SAKURA(30, -52, 150, 96, 23, L); SAKURA(790, -46, 140, 88, 29, L);
    // a few dark branches hanging into the frame from those canopies
    for (const [x0, y0, x1, y1] of [[120, 40, 210, 96], [150, 30, 250, 60], [700, 36, 600, 90], [690, 24, 560, 48]]) { line(L, x0, y0, x1, y1, hx('#1a1020')); line(L, x0 + 1, y0, x1 + 1, y1, hx('#2a1c30')); }
    for (const [x, y, r] of [[196, 88, 7], [232, 56, 6], [180, 70, 6], [612, 84, 7], [575, 46, 6], [640, 62, 6]]) {
      for (let yy = y - r; yy <= y + r; yy++) for (let xx = x - r; xx <= x + r; xx++) { const d = Math.hypot(xx - x, yy - y) / r + HS(xx, yy, 31) * 0.15; if (d > 1) continue; const l = (xx - x) / r * 0.6 - (yy - y) / r * 0.8; L.set(xx, yy, d > 0.9 ? hx('#4a2440') : l > 0.5 ? hx('#f4bcd0') : l > 0 ? hx('#d88aa4') : hx('#a85a7c')); }
    }
    return L;
  }
  ...
  let FAR = null, NEAR = null, FRONT = null;
  function paint() { if (!FAR) { FAR = paintFar(); NEAR = paintNear(); FRONT = paintFront(); windowSpans(); } return { far: FAR, near: NEAR, front: FRONT, frontRate: 1.25 }; }
  ...
  function anim(t, broken) {
    ...
    for (let k = 0; k < 4; k++) {
      ...
      for (const [x, y, w, top] of WINSPANS[wi] || []) out.push({ x, y, w, h: 1, col: on ? (top ? hx('#ffd98a') : hx('#e0b850')) : hx('#0f1130'), a: 1 });
    }
  ...
  root.STAGE = { W, H, VW, GROUND, FAR_W, FRONT_W, PXM, paint, half, anim, breakables, propImage, name: '東京鬼高校・夜の校門前', en: 'TOKYO ONI HIGH — NIGHT GATE' };
