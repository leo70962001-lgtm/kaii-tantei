...
(function (root) {
  'use strict';
  const PX = root.PX;
  const W = 720, H = 270, VW = 480, GROUND = 240, FAR_W = 600, FRONT_W = 800, PXM = 52;   // world px per metre on the fight plane
  let TREES = null;   // which near-layer pixels the sakura canopies cover (so lights behind them stay behind them)
  // ---------------------------------------------------------------- themes (「選擇場景」): the same street at three hours
  // Every colour a painter needs comes from TH so one drawing gives three stages: the night (the moon, blue), dusk (a warm
  // sky over the skyline, no lamps yet glowing strongly), and the neon rain (the cyberpunk pins: a dense skyline of
  // magenta / cyan windows, teal haze, rain in front of and behind the fighters).
  const THEMES = {
    night: { id: 'night', name: '夜の校門前', en: 'NIGHT GATE',
      sky: ['#07091c', '#0a0d26', '#10132f', '#161a3c', '#1d2048', '#262856', '#312f62', '#3d366c', '#4a3f74'], stars: 260, moon: true, sun: null,
      cloudA: { body: '#3a3872', shade: '#262458', lit: '#7a74b8', rim: '#c8c2f0' }, cloudB: { body: '#2e2c64', shade: '#1e1e4c', lit: '#5a56a0', rim: '#9a94d8', warm: '#5a3a5c' },
      cum: ['#dcd6f6', '#9690cc', '#6e68ae', '#2a2860', '#4c4894', '#3e3a80'], haze: ['#4c4180', '#6a4a78'],
      farTower: ['#3b3670', '#4a4684', '#8a86c0', 0.12], nearTower: ['#232650', '#343a6a', '#b8c4ff', 0.3, 0.5], midBuild: ['#1a1e48', '#2a2e5c', '#ffd98a', 0.22], dense: false,
      winLit: ['#ffd98a', '#e0b850'], sakura: ['#b04070', '#d8608c', '#f088ac', '#f8b4cc', '#ffe0ec'], sheen: '#5a4a8a', mist: '#3a3a70', rain: false },
    dusk: { id: 'dusk', name: '夕暮れの校門前', en: 'DUSK GATE',
      sky: ['#1a1440', '#2c1c58', '#48286a', '#6a3a72', '#8c4a6e', '#b06062', '#d08262', '#eaa472', '#f8c484'], stars: 30, moon: false, sun: [470, 158],
      cloudA: { body: '#5a3a70', shade: '#3a2450', lit: '#c07890', rim: '#ffd0b0' }, cloudB: { body: '#4a2e60', shade: '#2e1c44', lit: '#9a6080', rim: '#f0b0a0', warm: '#c06a60' },
      cum: ['#ffd8c0', '#e09aa0', '#a06080', '#4a2e60', '#7c4a7c', '#603a68'], haze: ['#8a5a78', '#c07a70'],
      farTower: ['#4a3468', '#5c4478', '#ffc890', 0.06], nearTower: ['#2e2050', '#443060', '#ffd0a0', 0.1, 0.2], midBuild: ['#2c1e4e', '#3e2c62', '#ffd0a0', 0.1], dense: false,
      winLit: ['#ffd8a0', '#e8b070'], sakura: ['#b04a78', '#d8709c', '#f094b8', '#ffb8d0', '#ffe0ea'], sheen: '#8a5a7a', mist: '#7a4a6a', rain: false },
    cyber: { id: 'cyber', name: '雨のネオン街', en: 'NEON RAIN',
      sky: ['#04080e', '#061018', '#081622', '#0a1c2a', '#0c2232', '#0e2838', '#123040', '#163848', '#1a4050'], stars: 60, moon: false, sun: null,
      cloudA: { body: '#123040', shade: '#0a1c2a', lit: '#2a5a6a', rim: '#5aa0b0' }, cloudB: { body: '#0e2838', shade: '#081622', lit: '#1e4a58', rim: '#3a8090', warm: '#4a2a5a' },
      cum: ['#7ac0d0', '#3a7080', '#1e4a58', '#0a1c2a', '#163848', '#123040'], haze: ['#1a4a5a', '#3a2a5a'],
  ...
  // a translucent colour over whatever is there: mixed into an opaque pixel, kept translucent over nothing (a layer is
  // drawn as one texture, so a see-through pixel would show the layer behind, not the road it was painted on)
  ...
  function paintFar() {
    ...
    // a big low moon with a wide soft halo (the night); at dusk the sun has just gone behind the skyline
    ...
    if (TH.moon) for (let y = my - 70; y <= my + 70; y++) for (let x = mx - 70; x <= mx + 70; x++) {
    ...
    if (TH.moon) for (const [dx, dy, r] of [[-10, -6, 5], [9, 6, 4], [-3, 13, 3], [12, -12, 3], [-13, 9, 3]]) for (let y = -r; y <= r; y++) for (let x = -r; x <= r; x++) {
    ...
    const C1 = pal(TH.cloudA), C2 = pal(TH.cloudB);
    ...
    // cauliflower cumulus (the reference's clouds): lobes on a flat base, the side toward the moon bright, the underside
    // dark, a grainy edge — one beside the moon, one over the apartment, a small one above the gate
    ...
    const tower = (x0, w, top, col, win, lit, wcol, edge) => {
      for (let y = top; y < GROUND; y++) for (let x = x0; x < x0 + w; x++) L.set(x, y, x === x0 ? edge : col);
      for (let x = x0; x < x0 + w; x++) L.set(x, top, edge);                                     // the roof edge
      if (win) for (let y = top + 3; y < GROUND - 6; y += 5) for (let x = x0 + 2; x < x0 + w - 3; x += 4) {   // a grid of 2×3 windows
        const on = HS(x, y, 5) < lit; const c = on ? (HS(x, y, 6) > 0.7 ? hx('#ffd98a') : wcol) : 0;
        if (c) { L.set(x, y, c); L.set(x + 1, y, c); L.set(x, y + 1, c); L.set(x + 1, y + 1, c); L.set(x, y + 2, c); L.set(x + 1, y + 2, c); }
        else { L.set(x, y + 2, edge); L.set(x + 1, y + 2, edge); }
      }
      const r = HS(x0, top, 8);
      if (r > 0.7) { rect(L, x0 + (w >> 1) - 1, top - 6, 2, 6, edge); L.set(x0 + (w >> 1) - 1, top - 7, hx('#ff5a5a')); }        // an antenna with a warning light
      else if (r > 0.45) { rect(L, x0 + 3, top - 3, 6, 3, edge); rect(L, x0 + w - 8, top - 2, 5, 2, edge); }                   // rooftop boxes
    };
    for (let k = 0; k < 26; k++) {
      const x0 = Math.floor(HS(k, 7, 3) * FAR_W), w = 15 + Math.floor(HS(k, 8, 3) * 40), top = 130 + Math.floor(HS(k, 9, 3) * 45);
      tower(x0, w, top, hx(TH.farTower[0]), true, TH.farTower[3], hx(TH.farTower[2]), hx(TH.farTower[1]));
    }
    if (TH.dense) for (let k = 0; k < 34; k++) {   // the neon city: a second, taller rank of towers with neon-lit windows
      const x0 = Math.floor(HS(k, 27, 3) * FAR_W), w = 12 + Math.floor(HS(k, 28, 3) * 26), top = 40 + Math.floor(HS(k, 29, 3) * 90);
      tower(x0, w, top, hx(TH.farTower[0]), true, 0.35, hx(HS(k, 30, 3) > 0.5 ? '#ff5ad0' : '#5ad0ff'), hx(TH.farTower[1]));
    }
    ...
    for (let k = 0; k < 18; k++) {
      ...
      tower(x0, w, top, hx(TH.nearTower[0]), true, TH.nearTower[3], hx(TH.nearTower[2]), hx(TH.nearTower[1]));
      if (HS(k, 24, 4) > 1 - TH.nearTower[4] * 0.5) for (let y = top + 2; y < GROUND; y += 2) { L.set(x0 + 2, y, hx('#5ad0ff')); L.set(x0 + w - 3, y, hx('#5ad0ff')); }   // a glass tower's light strips
  ...
  function paintNear() {
    ...
    for (let row = 0; row < 3; row++) for (let x = 292; x < 484; x += 12) {
      ...
      rect(L, x, y, 7, 9, lit ? hx(TH.winLit[0]) : hx('#0f1330')); if (lit) rect(L, x, y + 5, 7, 4, hx(TH.winLit[1])); else L.set(x + 1, y + 1, hx('#2a3060'));
    }
    ...
    const win = (x, y, lit, curtain) => {
      ...
      rect(L, x, y, 21, 25, lit ? hx(TH.winLit[0]) : hx('#0f1130'));
      if (lit) { rect(L, x, y + 13, 21, 12, hx(TH.winLit[1])); if (curtain) { rect(L, x, y, 6, 25, hx('#f0c890')); rect(L, x + 15, y, 6, 25, hx('#f0c890')); } }
    ...
    for (let x = AX0; x < AX1; x += 5) rect(L, x, 58, 1, 6, hx('#3a3e70')); rect(L, AX0, 58, AX1 - AX0, 1, hx('#4a4f80'));   // the roof railing
    for (const [x, y] of [[52, 96], [164, 96], [108, 158]]) { rect(L, x, y, 10, 7, hx('#3a3e70')); rect(L, x, y, 10, 1, hx('#5a5f8a')); for (let i = 1; i < 6; i += 2) rect(L, x + 1, y + i, 8, 1, hx('#262a50')); }   // AC units
    rect(L, 18, 100, 32, 1, hx('#8a90b0')); rect(L, 22, 101, 6, 8, hx('#e8e8f0')); rect(L, 24, 101, 2, 3, hx('#c8c8d8')); rect(L, 32, 101, 5, 9, hx('#7ad4ff')); rect(L, 41, 101, 4, 6, hx('#ffd6e0'));   // laundry on a pole
    rect(L, 226, 68, 20, 120, hx('#161a3e')); for (const y of [76, 108, 140]) { rect(L, 230, y, 12, 9, hx('#2c3060')); rect(L, 231, y + 1, 10, 7, hx('#6a5a30')); rect(L, 232, y + 2, 8, 5, hx('#a89050')); }   // the stairwell's dim landing windows
    rect(L, 12, 68, 2, 120, hx('#2a2e5c')); rect(L, 12, 68, 1, 120, hx('#3a3e70'));                                       // a drain pipe
    glow(L, 237, 100, 30, 44, '#ff5ad0', 60, 1.5);
    rect(L, 230, 72, 14, 56, hx('#3a1040')); rect(L, 231, 73, 12, 54, hx('#ff5ad0')); rect(L, 232, 74, 10, 52, hx('#c0187a'));   // the vertical neon sign
    for (let k = 0; k < 4; k++) { const y = 77 + k * 13; rect(L, 234, y, 6, 1, hx('#ffd0f0')); rect(L, 236, y + 1, 1, 8, hx('#ffd0f0')); rect(L, 233, y + 4, 8, 1, hx('#ffd0f0')); rect(L, 234, y + 8, 6, 1, hx('#ffd0f0')); }   // its glyphs
    rect(L, 232, 128, 10, 2, hx('#3a1040')); rect(L, 236, 130, 2, 6, hx('#3a1040'));
    ...
    // a convenience store behind the wall at the right (×0.5): the striped sign and the top of its lit glass front show
    // above the wall (the reference's storefront), its light spilling over the coping
    rect(L, 626, 150, 94, 38, hx('#1e2250')); rect(L, 626, 150, 94, 2, hx('#2c3060'));
    rect(L, 630, 154, 86, 12, hx('#f0f0f8')); rect(L, 630, 154, 86, 3, hx('#3ab54a')); rect(L, 630, 163, 86, 3, hx('#1f6fe0'));
    for (const [x, w] of [[636, 5], [643, 5], [651, 7], [661, 5], [668, 6], [677, 5], [685, 6], [694, 5], [702, 7]]) rect(L, x, 158, w, 4, hx('#1a1a2a'));   // the sign's lettering
    rect(L, 630, 167, 86, 21, hx('#ffe9b0')); rect(L, 630, 167, 86, 1, hx('#8a90b0'));
    for (let y = 172; y < 188; y += 5) { rect(L, 634, y, 26, 1, hx('#8a7a50')); rect(L, 678, y, 34, 1, hx('#8a7a50')); for (let x = 635; x < 659; x += 3) rect(L, x, y - 3, 2, 3, [hx('#e84a5f'), hx('#5ad0ff'), hx('#7ae06a'), hx('#ffd24a')][(x + y) % 4]); for (let x = 679; x < 711; x += 3) rect(L, x, y - 3, 2, 3, [hx('#ff8a3a'), hx('#c9cce6'), hx('#ff5ad0'), hx('#7ad4ff')][(x + y) % 4]); }   // shelves of goods
    rect(L, 662, 167, 2, 21, hx('#8a90b0')); rect(L, 674, 167, 2, 21, hx('#8a90b0')); rect(L, 664, 178, 10, 10, hx('#fff4d0'));   // the sliding door
    glow(L, 673, 190, 70, 26, '#ffe0a0', 34, 1.4);
    glow(L, 700, 143, 22, 12, '#5ad0ff', 50, 1.5); rect(L, 690, 138, 20, 10, hx('#083048')); rect(L, 691, 139, 18, 8, hx('#5ad0ff')); rect(L, 693, 141, 3, 4, hx('#083048')); rect(L, 698, 141, 3, 4, hx('#083048')); rect(L, 703, 141, 3, 4, hx('#083048'));   // a cyan "24h" neon
    ...
    TREES = layer(W, H);
    // a cherry tree the way the references draw it: a dark trunk and twisting limbs (recursive, three levels) that stay
    // visible inside the crown, and the crown as many small clusters of flowers hung along the twigs — sky shows in the
    // gaps; tones are the night's neon-lit pinks (deep magenta in shadow, pale pink toward the light). opts: ang/len of
    // the first limbs, trunk (draw one down to the wall top), light (x direction of the light: +1 from the right)
    const sakura = (cx, cy, rx, ry, seed, T, opts) => {
      T = T || L; opts = opts || {}; const near = T === L, light = opts.light === undefined ? 1 : opts.light;
      const BARK = hx('#241832'), BARK2 = hx('#463050'), BARK3 = hx('#140c1c');
      const mark = (x, y) => { if (near) TREES.set(x, y, 1); };
      const limbLine = (x0, y0, x1, y1, w) => {
        for (let o = -w / 2; o < w / 2; o++) { const c = o < -w / 4 - 0.5 ? BARK2 : o > w / 4 ? BARK3 : BARK; line(T, x0 + o, y0, x1 + o, y1, c); if (near) line(TREES, x0 + o, y0, x1 + o, y1, 1); }
      };
      const limbs = [];
      const grow = (x, y, ang, len, w, depth, k) => {
        ...
        for (let i = 0; i < n; i++) {
          const spread = 0.45 + HS(k * 7 + i, depth, seed) * 0.5, a = ang + (i - (n - 1) / 2) * spread + (HS(k * 3 + i, depth + 9, seed) - 0.5) * 0.5;
          grow(x1, y1, a, len * (0.6 + HS(k + i, depth + 3, seed) * 0.25), Math.max(1, w * 0.62), depth - 1, k * 4 + i + 1);
        }
      };
      if (opts.trunk !== false) {
        const base = cy + ry * 0.38;
        for (let y = base; y < 190; y++) { const w = 6 + (y - base) / 40; for (let i = -w / 2; i < w / 2; i++) { T.set(cx + i + Math.round(Math.sin(y * 0.15 + seed) * 1.2), y, i < -w / 4 ? BARK2 : i > w / 4 ? BARK3 : BARK); mark(cx + i + Math.round(Math.sin(y * 0.15 + seed) * 1.2), y); } }
        grow(cx, base, -Math.PI / 2 + (HS(1, 1, seed) - 0.5) * 0.5, ry * 0.42, 5, 3, 1);
        grow(cx, base + 6, -Math.PI / 2 - 0.9 * (HS(2, 1, seed) > 0.5 ? 1 : -1), ry * 0.32, 3, 2, 9);
      } else grow(cx, cy, opts.ang || Math.PI / 2, opts.len || ry * 0.45, 5, 3, 1);
      limbs.sort((p, q) => q[4] - p[4]);   // thick first, twigs on top
      for (const [x0, y0, x1, y1, w] of limbs) limbLine(x0, y0, x1, y1, w);
      // flowers: small clusters along the twigs (the thinner the limb the more it carries), kept inside the crown ellipse
      const TONES = TH.sakura.map(hx), WHITE = hx('#fff2f8'), GAP = hx('#3a2240');   // the theme's pinks (night: neon-lit, dusk: warm)
      const cluster = (fx, fy, r, k) => {
        for (let y = Math.floor(fy - r); y <= fy + r; y++) for (let x = Math.floor(fx - r); x <= fx + r; x++) {
          ...
          const n = HS(x, y, seed + 6 + k);
          if (n < 0.13) continue;                                        // gaps between the flowers
          const l = ((x - fx) / r) * 0.55 * light - ((y - fy) / r) * 0.75 + (HS(x, y, seed + 7) - 0.5) * 0.5;
          const c = n > 0.96 ? WHITE : l > 0.55 ? TONES[4] : l > 0.15 ? TONES[3] : l > -0.25 ? TONES[2] : l > -0.6 ? TONES[1] : TONES[0];
          T.set(x, y, c); mark(x, y);
        }
      };
      let k = 0;
      ...
      // a few dark gaps so the crown reads as clusters, not a blob
      for (let g = 0; g < 8; g++) { const gx = cx + (HS(g, 31, seed) - 0.5) * rx * 1.4, gy = cy + (HS(g, 32, seed) - 0.5) * ry * 1.2; for (let y = gy - 2; y <= gy + 2; y++) for (let x = gx - 3; x <= gx + 3; x++) if (T.get(x, y) && HS(x, y, seed + 33) < 0.5) T.set(x, y, GAP); }
    };
    sakura(118, 96, 70, 52, 13); sakura(586, 90, 78, 58, 17, L, { light: -1 }); sakura(704, 112, 54, 42, 19);
    SAKURA = sakura;
    ...
    // the street mirrored in the wet asphalt: the wall, gate and pillars flipped, dim, broken by ripples
    for (let y = 255; y < H; y++) {
      const t = (y - 255) / (H - 255), sy = GROUND - 1 - Math.round((y - 255) * 1.15), off = Math.round(Math.sin(y * 1.9) * 1.5 + Math.sin(y * 0.7) * 1.2);
      for (let x = 0; x < W; x++) { const c = L.get(x + off, sy); if (c && dith(x, y, 0.62 - t * 0.4)) mixPx(L, x, y, c, Math.round(82 - t * 46)); }
    }
    for (let y = 255; y < H; y++) for (let x = 0; x < W; x++) if (HS(x, y, 47) < 0.08 + (y - 255) / 60) mixPx(L, x, y, hx(TH.sheen), 22 + Math.round((y - 255) * 1.5 * (TH.rain ? 1.6 : 1)));
    for (let y = 255; y < H; y++) for (let x = 555; x < 705; x++) { const d = Math.hypot((x - 630) / 72, (y - 262) / 9); if (d < 1 && dith(x, y, (1 - d) * 0.7)) mixPx(L, x, y, hx('#8a7ab8'), 70); }   // a puddle
    // the street lamp: its cone of light down to the sidewalk, and its reflection on the tiles and the road
    for (let y = 66; y < GROUND; y++) { const t = (y - 66) / (GROUND - 66), hw = 4 + t * 34; for (let x = Math.round(569 - hw); x <= Math.round(569 + hw); x++) { const e = 1 - Math.abs(x - 569) / hw, a = 30 * (1 - t * 0.85) * e; if (a >= 4 && dith(x, y, Math.min(1, a / 10))) mixPx(L, x, y, hx('#ffd890'), Math.max(8, Math.round(a))); } }
    ...
    streak(L, 228, 246, 255, H, '#ff5ad0', 70); streak(L, 224, 250, GROUND, 252, '#ff5ad0', 30);   // the neon's reflection
    streak(L, 688, 712, 255, H, '#5ad0ff', 50);
    // fallen petals on the sidewalk and the road, thickest under the trees (the references' carpets of petals)
    const PET = [hx('#ff9cd6'), hx('#f070bc'), hx('#ffd0ea'), hx('#d04a98')];
    for (let y = GROUND + 1; y < H; y++) for (let x = 0; x < W; x++) {
      let dens = 0.012; for (const tx of [118, 586, 704]) dens += 0.08 * Math.max(0, 1 - Math.abs(x - tx) / 110);
      if (y > 252) dens *= 0.45;
      if (HS(x, y, 61) < dens) { const c = PET[Math.floor(HS(x, y, 62) * 4)]; L.set(x, y, c); if (HS(x, y, 63) < 0.4) L.set(x + 1, y, c); }
    }
    return L;
  }
  ...
  let SAKURA = null;
  // the front layer: sakura branches reaching into the top corners, nearer than the fighters (the reference frames its
  // city with blossoms); 800 px wide because it scrolls faster (×1.25) than the street
  function paintFront() {
    const L = layer(FRONT_W, H);
    SAKURA(10, -24, 230, 120, 23, L, { trunk: false, ang: Math.PI * 0.36, len: 52, light: 1 });
    SAKURA(790, -20, 220, 112, 29, L, { trunk: false, ang: Math.PI * 0.64, len: 50, light: -1 });
    return L;
  }
  // the lit panes of the apartment windows, minus whatever a canopy covers: row spans in stage coordinates
  const WINSPANS = [];
  function windowSpans() {
    for (const [wx, wy] of WINDOWS) {
      const spans = [];
      for (let y = wy; y < wy + 25; y++) {
        if (y === wy + 12) continue;                        // the sash stays painted
        let x0 = -1;
        for (let x = wx; x <= wx + 21; x++) {
          const pane = x < wx + 21 && x !== wx + 10 && !TREES.get(x, y);
          if (pane && x0 < 0) x0 = x;
          if (!pane && x0 >= 0) { spans.push([x0, y, x - x0, y < wy + 13]); x0 = -1; }
        }
      }
      WINSPANS.push(spans);
    }
  }
  // the MID layer (parallax 0.6, between the skyline and the street): a rank of mid-distance blocks (×0.25) with window
  // grids, the elevated railway the train runs on, and a band of mist — what gives the picture its depth between the
  // hazy far city and the wall (「雲跟場景的深度」)
  const MID_W = 660, MID_RATE = 0.6;
  function paintMid() {
    const L = layer(MID_W, H); const [body, edge, win, lit] = TH.midBuild.map((v, i) => (i < 3 ? hx(v) : v));
    for (let k = 0; k < 22; k++) {   // mid-rise blocks whose tops show above the school and the wall (the street hides the rest)
      const x0 = Math.floor(HS(k, 41, 5) * MID_W), w = 34 + Math.floor(HS(k, 42, 5) * 50), top = (TH.dense ? 8 : 26) + Math.floor(HS(k, 43, 5) * 56);
      for (let y = top; y < 200; y++) for (let x = x0; x < x0 + w; x++) L.set(x, y, x === x0 || y === top ? edge : dith(x, y, (y - top) / 200 + HS(x, y, 44) * 0.1) ? body : hx(TH.midBuild[0]));
      for (let y = top + 6; y < 190; y += 12) for (let x = x0 + 4; x < x0 + w - 6; x += 9) {
        const on = HS(x, y, 45) < lit; if (on) { rect(L, x, y, 5, 6, win); rect(L, x, y + 3, 5, 3, hx(TH.winLit[1])); } else { rect(L, x, y, 5, 6, hx('#0a0e24')); L.set(x + 1, y + 1, edge); }
      }
      if (HS(k, 46, 5) > 0.6) { rect(L, x0 + 6, top - 5, 8, 5, edge); rect(L, x0 + w - 12, top - 3, 6, 3, edge); }
      if (TH.dense && HS(k, 47, 5) > 0.5) { const sx = x0 + 4, sc = HS(k, 48, 5) > 0.5 ? hx('#ff5ad0') : hx('#5ad0ff'); rect(L, sx, top + 2, w - 8, 3, sc); glow(L, sx + (w - 8) / 2, top + 3, w / 2 + 8, 8, HS(k, 48, 5) > 0.5 ? '#ff5ad0' : '#5ad0ff', 50, 1.4); }
    }
    // the elevated railway: a viaduct with pillars, a fence, catenary posts
    rect(L, 0, 72, MID_W, 3, hx('#4a4f80')); rect(L, 0, 75, MID_W, 9, hx('#2e3358')); rect(L, 0, 84, MID_W, 2, hx('#1e2244'));
    for (let x = 0; x < MID_W; x += 7) rect(L, x, 68, 1, 4, hx('#5a5f8a')); rect(L, 0, 68, MID_W, 1, hx('#5a5f8a'));
    for (let x = 20; x < MID_W; x += 64) { rect(L, x, 86, 8, 114, hx('#262a50')); rect(L, x, 86, 2, 114, hx('#3a3e70')); rect(L, x + 3, 58, 1, 14, hx('#5a5f8a')); rect(L, x + 1, 58, 6, 1, hx('#5a5f8a')); }
    // the mist band in front of the blocks
    for (let y = 96; y < 150; y++) for (let x = 0; x < MID_W; x++) { const t = Math.sin((y - 96) / 54 * Math.PI); if (HS(x >> 1, y, 49) < t * 0.5) mixPx(L, x, y, hx(TH.mist), Math.round(30 + t * 60)); }
    return L;
  }
  const PAINTED = {};
  function paint(id) {
  ...
  function anim(t, broken) {
    ...
    // windows switching (the apartment block, 21×25 panes) — only the parts of a pane no sakura canopy covers, so the
    // light stays behind the trees (「窗子的燈光在櫻花樹後」)
    for (let k = 0; k < 4; k++) {
      const slot = Math.floor(s / 1.1) + k * 7, on = H(slot, 9, 21) > 0.5, wi = Math.floor(H(slot, 10, 21) * WINDOWS.length) % WINDOWS.length;
      for (const [x, y, w, top] of WINSPANS[wi] || []) out.push({ x, y, w, h: 1, col: on ? (top ? TH.winLit[0] : TH.winLit[1]) : '#0f1130', a: 1 });
    }
    ...
    // the train on the elevated railway every 22 s (mid layer): six cars, lit windows, a headlight — right to left
    const tp = (s % 22) / 22;
    if (tp < 0.32) {
      const u = tp / 0.32, tx0 = MID_W + 40 - u * (MID_W + 260);
      for (let c = 0; c < 6; c++) {
        const cx = tx0 + c * 34;
        out.push({ mid: true, x: cx, y: 60, w: 32, h: 12, col: '#c9cce6', a: 1 }); out.push({ mid: true, x: cx, y: 60, w: 32, h: 2, col: '#e8e8f0', a: 1 }); out.push({ mid: true, x: cx, y: 69, w: 32, h: 3, col: '#3a3e66', a: 1 });
        for (let i = 0; i < 5; i++) out.push({ mid: true, x: cx + 3 + i * 6, y: 62, w: 4, h: 5, col: TH.rain ? '#ffb0e0' : '#ffe6a0', a: 1 });
        if (c === 0) out.push({ mid: true, x: cx - 2, y: 64, w: 2, h: 3, col: '#ffffff', a: 1 });
      }
    }
    // rain (the neon theme): streaks behind the wall and a few in front of the fighters, each on its own timing
    if (TH.rain) for (let k = 0; k < 110; k++) {
      const spd = 160 + H(k, 51, 21) * 120, x = ((H(k, 52, 21) * (W + 60) + s * 14) % (W + 60)) - 30, y = ((H(k, 53, 21) * 320 + s * spd) % (H + 40)) - 20, front = k % 4 === 0;
      out.push(front ? { x: Math.round(x), y: Math.round(y), w: 1, h: 9, col: '#bfe0ff', a: 0.5, front: true } : { mid: true, x: Math.round(x * 1.2), y: Math.round(y), w: 1, h: 7, col: '#9ad0e0', a: 0.35 });
    }
  ...
  root.STAGE = { W, H, VW, GROUND, FAR_W, FRONT_W, MID_W, MID_RATE, PXM, paint, setTheme, THEMES, THEME_IDS, theme: () => TH, half, anim, breakables, propImage, name: '東京鬼高校・夜の校門前', en: 'TOKYO ONI HIGH — NIGHT GATE' };
